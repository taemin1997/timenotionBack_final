...
public class FollowPage {
    ...
    public FollowPage(FollowCriteria followCriteria , int total) {
        this(followCriteria, total, 5);
    }


    public FollowPage(FollowCriteria followCriteria, int total, int pageCount) {
        ...
        //ceil() : 올림처리 메소드
        this.endPage = (int)(Math.ceil(followCriteria.getPage()/(double)pageCount)) * pageCount;
        this.startPage = endPage - pageCount +1;

        //게시글 전체 수로 실제 마지막 페이지를 구한다
        this.realEnd = (int)Math.ceil((double)total/followCriteria.getAmount());
